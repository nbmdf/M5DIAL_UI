...
float loadLimit = 10.0;            
float loadDemandPower = 5.0;       

// Color definitions
#define COLOR_BG 0x0000
#define COLOR_LOAD 0xF81F      
...
void setup_wifi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    CLIENT_ID = "M5Dial-Load-" + String(macStr);
    
    publish_topic = "dial/load/" + DEVICE_ID;
...
void loop() {
  ...
  if (newPosition != oldPosition) {
    if (isEditing) {
      ...
      int newIndex = (newPosition / 4) % LOAD_SETTING_COUNT;
      int oldIndex = (oldPosition / 4) % LOAD_SETTING_COUNT;
      if (newIndex != oldIndex) {
        currentSettingIndex = ((newIndex % LOAD_SETTING_COUNT + LOAD_SETTING_COUNT) % LOAD_SETTING_COUNT);
        oldPosition = newPosition;
        drawLoadPage();
      }
    }
  ...
  if (M5Dial.BtnA.wasPressed()) {
    if (isEditing && currentSettingIndex != LOAD_CONFIRM) {
      isEditing = false;
      publishLoadData();
      drawLoadPage();
    } else if (currentSettingIndex == LOAD_CONFIRM) {
      publishLoadData();
      M5Dial.Display.fillScreen(COLOR_BG);
...
void adjustValue(int delta) {
  ...
  switch (currentSettingIndex) {
    case LOAD_LIMIT:
      loadLimit -= delta * increment;
      loadLimit = constrain(loadLimit, -100.0, 0.0);
      if (loadDemandPower < loadLimit) {
        loadDemandPower = loadLimit;
      }
      break;
    case LOAD_DEMAND_POWER:
      loadDemandPower -= delta * increment;
      loadDemandPower = constrain(loadDemandPower, loadLimit, 0.0);
      break;
  }
}
...
void drawLoadPage() {
  ...
  M5Dial.Display.fillRect(0, 0, 240, 40, COLOR_LOAD);
  M5Dial.Display.setTextColor(COLOR_BG);
  M5Dial.Display.setTextSize(1.5);
  M5Dial.Display.drawString("Load Settings", 120, 20);
  ...
  drawSettingItem("Limit (kW)", String(loadLimit, 1), yStart, 
                  currentSettingIndex == LOAD_LIMIT, isEditing);

  M5Dial.Display.setTextSize(1.5);
  M5Dial.Display.setTextColor(COLOR_TEXT);
  drawSettingItem("Demand (kW)", String(loadDemandPower, 1), yStart + ySpacing, 
                  currentSettingIndex == LOAD_DEMAND_POWER, isEditing);

  M5Dial.Display.setTextSize(1.5);
  M5Dial.Display.setTextColor(COLOR_TEXT);
  M5Dial.Display.drawString(String(loadDemandPower, 1) + " kW", 120, yStart + ySpacing * 2);
  
  M5Dial.Display.setTextSize(1.5);
  M5Dial.Display.setTextColor(currentSettingIndex == LOAD_CONFIRM ? COLOR_LOAD : COLOR_TEXT);
  M5Dial.Display.drawString("Send Now", 120, yStart + ySpacing * 3);
...
void drawSettingItem(const char* label, String value, int y, bool selected, bool editing) {
  ...
  if (selected) {
    if (editing) {
      ...
      textColor = valueColor = COLOR_LOAD;
    }
  }
